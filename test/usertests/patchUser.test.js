...
describe("When patching a user", () => {
  ...
  it("then it should update user in storage with new details", async () => {
    ...
    expect(update.mock.calls[0][0]).toBe(
      "9b543631-884c-4b39-86d5-311ad5fc6cce",
    );
  ...
  it("then it should send 400 with error message if body has unknown property", async () => {
    ...
    expect(res._getData()).toBe(
      "Unpatchable property bad. Allowed properties given_name,family_name,email,job_title,phone_number,legacyUsernames",
    );
    expect(res._isEndCalled()).toBe(true);
  });
  ...
  it("then it should send 400 with error message if body has unpatchable property", async () => {
    ...
    expect(res.statusCode).toBe(400);
    expect(res._getData()).toBe(
      "Unpatchable property sub. Allowed properties given_name,family_name,email,job_title,phone_number,legacyUsernames",
    );
    expect(res._isEndCalled()).toBe(true);
  });
  ...
  it("then it should send 400 with error message if body has no properties", async () => {
    ...
    expect(res.statusCode).toBe(400);
    expect(res._getData()).toBe(
      "Must specify at least one property to update. Allowed properties given_name,family_name,email,job_title,phone_number,legacyUsernames",
    );
    expect(res._isEndCalled()).toBe(true);
  });
