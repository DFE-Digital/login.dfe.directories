...
describe("When deleting a user code", () => {
  ...
  it("then the params are passed to the storage provider", async () => {
    ...
    expect(redisUserCodeStorage.deleteUserCode.mock.calls[0][0]).toBe(
      "7654321",
    );
    expect(redisUserCodeStorage.deleteUserCode.mock.calls[0][1]).toBe(
      "PasswordReset",
    );
    expect(redisUserCodeStorage.deleteUserCode.mock.calls[0][2]).toBe(
