...
jest.mock('../../../../../src/app/user/adapter/userSequelizeHelpers/findUserByEntraOidHelper');
jest.mock('../../../../../src/app/user/adapter/userSequelizeHelpers/findUserByIdHelper');
...
describe('linkUserWithEntraOid function', () => {
  ...
  it('should return null if a user is not found', async () => {
    ...
    const result = await linkUserWithEntraOid('uid', 'entra-id', undefined, undefined, correlationId);
  ...
  it('should create a new user entity with given_name changed', async () => {
    findUserById.mockResolvedValue({
      update: jest.fn().mockImplementation((updatedFields) => ({ ...fakeExistingUser, ...updatedFields })),
    });
    ...
    const result = await linkUserWithEntraOid(userId, entraOid, firstName, undefined, correlationId);
  ...
  it('should create a new user entity with family_name changed', async () => {
    ...
    const result = await linkUserWithEntraOid(userId, entraOid, undefined, lastName, correlationId);
  ...
  it('should handle the raising of an exception', async () => {
    ...
    await expect(linkUserWithEntraOid('uid', 'entra-id', undefined, undefined, correlationId)).rejects.toThrow();
    expect(logger.error).toHaveBeenCalledWith('linkUserWithEntra failed for request testCorrelationId error: Error', { correlationId });
  });
});
