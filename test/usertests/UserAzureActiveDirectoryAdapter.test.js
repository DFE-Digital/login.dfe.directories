...
jest.mock('activedirectory', () => {
  ...
  const findUserStub = jest.fn().mockImplementation((options, userName, callback) => {
    callback(null, user);
  });
  const authenticateStub = jest.fn().mockImplementation((userName, password, callback) => {
    callback(null, true);
  });
...
describe('When using the UserAzureActiveDirectoryAdapter', () => {
  ...
  describe('and finding user by email', () => {
    it('the user is read from active directory', async () => {
      ...
      expect(activeDirectory().findUser.mock.calls[0][1]).toBe(expectedUserName);
  ...
  describe('and authenticating', () => {
    it('then the username and password are checked against the active directory', async () => {
      ...
      const adapter = require('./../../src/app/user/adapter/UserAzureActiveDirectoryAdapter');
      const actual = await adapter.authenticate(expectedUserName, expectedPassword);

      expect(activeDirectory().authenticate.mock.calls).toHaveLength(1);
      expect(activeDirectory().authenticate.mock.calls[0][0]).toBe(expectedUserName);
      expect(activeDirectory().authenticate.mock.calls[0][1]).toBe(expectedPassword);
      expect(actual).not.toBeNull();
