...
describe("When getting a user code", () => {
  ...
  it("then if no code is found for the uid it is checked against the find by email", async () => {
    ...
    expect(redisUserCodeStorage.getUserCodeByEmail.mock.calls[0][0]).toBe(
      "8332e8c9-d665-4ca8-8eed-379ce773e0f6",
    );
    expect(redisUserCodeStorage.getUserCodeByEmail.mock.calls[0][1]).toBe(
      "PasswordReset",
    );
  ...
  it("then the parameters are passed to the storage provider", async () => {
    ...
    expect(redisUserCodeStorage.getUserCode.mock.calls[0][0]).toBe(
      "8332e8c9-d665-4ca8-8eed-379ce773e0f6",
    );
    expect(redisUserCodeStorage.getUserCode.mock.calls[0][1]).toBe(
      "PasswordReset",
    );
    expect(redisUserCodeStorage.getUserCode.mock.calls[0][2]).toBe(
