...
describe('findUserByEntraOidHelper function', () => {
  ...
  it('should return user entity when a user is found', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('Get user by entraOid for request', { correlationId });
  ...
  it('should log an error and throw if there is a database error', async () => {
    ...
    await expect(findUserByEntraOidHelper(entraOid, correlationId)).rejects.toThrow(errorMessage);
