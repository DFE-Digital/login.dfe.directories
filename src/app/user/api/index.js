...
const routeExport = () => {
  ...
  router.get(
    "/users/by-legacyusername/:id",
    asyncWrapper(findByLegacyUsername),
  );
  router.get("/users/:id", asyncWrapper(find));
  router.get(
    "/users/:uid/legacy-username",
    asyncWrapper(findLegacyUsernamesById),
  );
  router.post("/users/:uid/match-phrase", asyncWrapper(matchedPassphrase));
  router.get(
    "/users/:uid/password-policies",
  ...
  router.post(
    "/:directoryId/user/:id/changepassword",
    deprecateWith("/users/:id/changepassword"),
